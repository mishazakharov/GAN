...
class Generator(torch.nn.Module):
    ...
    def __init__(self, ch=64, image_dim=3, interpolation: bool = True) -> None:
        ...
        # Entry
        self.downsample_1 = torch.nn.Conv2d(image_dim, ch, kernel_size=4, stride=2, padding=1)  # 32 x 256
        # Main encoder
        self.downsample_2 = DownSample(ch, ch * 2)  # 16 x 128
        self.downsample_3 = DownSample(ch * 2, ch * 4)  # 8 x 64
        self.downsample_4 = DownSample(ch * 4, ch * 8)  # 4 x 32
        self.downsample_5 = DownSample(ch * 8, ch * 8)  # 2 x 16
        self.downsample_6 = DownSample(ch * 8, ch * 8)  # 1 x 8
        self.downsample_7 = DownSample(ch * 8, ch * 8, kernel_size=(1, 4), padding=(0, 1))  # 1 x 4
        self.downsample_8 = DownSample(ch * 8, ch * 8, kernel_size=(1, 4), padding=(0, 0), use_bn=False)  # 1 x 1

        # Decoding path
        self.upsample_1 = UpSample(ch * 8, ch * 8, kernel_size=(1, 4), padding=(0, 0))
        self.upsample_2 = UpSample(ch * 8 * 2, ch * 8, kernel_size=(1, 4), padding=(0, 1))
...
class DownSample(torch.nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int, kernel_size: int = 4, stride: int = 2,
                 padding: int = 1, inplace=False, use_bn=True) -> None:
        super().__init__()
        self.downsample = torch.nn.Conv2d(in_ch, out_ch, kernel_size=kernel_size, stride=stride, padding=padding)
...
if __name__ == "__main__":
    ...
    output = model_1(dummy_label)
    output_disc = model(output, dummy_label)
    print(output.shape)
