...
while True:
    ...
    for i, batch in enumerate(data_loader):
        ...
        D_x = torch.nn.functional.softplus(-D_x).mean()
        D_x.backward(retain_graph=True)

        grad_real = torch.autograd.grad(outputs=D_x.sum(), inputs=real_batch, create_graph=True)[0]
        grad_penalty = (grad_real.view(grad_real.shape[0], -1).norm(2, dim=1) ** 2).mean()
        grad_penalty = 10 / 2 * grad_penalty
        grad_penalty.backward()

        D_G_z1 = torch.nn.functional.softplus(D_G_z1).mean()
        D_G_z1.backward()

        # gp = gradient_penalty(gradient)
        # D_error, wasserstein_D = get_crit_loss(D_G_z1, D_x, gp, c_lambda)
        ...
        # Update optimizer
        optimizer_D.step()
        D_error_mean += (D_x + D_G_z1).item() / disc_repeats
        ...
        if global_step % disc_repeats == 0:
            ...
            G_error = torch.nn.functional.softplus(-D_G_z2).mean()
