    ...
    while True:
        for i, batch in enumerate(data_loader):
            ...
            if global_step % 100 == 0:
                ...
                write_logs({"Global Step": global_step, "G_Loss": G_error.mean().item(),
                            "D_Loss": D_error.mean().item(), "D(X)": D_x.mean(),
                            "D(G(Z))": [D_G_z1.mean().item(), D_G_z2.mean().item()]}, tensorboard, global_step)
